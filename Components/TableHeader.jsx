...
const TableHeader = ({ header }) => {
    ...
    return (
        <div className={`source_reverse target ${styles.HeaderDiv}`} style={{ zIndex: 2 }} >
            <div className={styles.rTable}>
                <div className={styles.rTableHeading}>

                    {
                        header.map((value) => {
                            return (
                                <div className={`${styles.rTableHead} ${value.stick}`}
                                    style={{ minWidth: `${value.width}`, maxWidth: `${value.maxWidth}` }} key={value.id}>
                                    {
                                        value.headerName

                                    }
                                    <br />
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" height="16" width="16" viewBox="0 0 24 24" color="neutral4" className="" >
